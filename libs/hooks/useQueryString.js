...
function useQueryString(config = defaultConfig) {
  ...
  // ORDER
  const getOrderBy = useMemo(() => getUrlParams(config.key.orderBy), [
    getUrlParams,
    config.key.orderBy,
  ])
  ...
  // SORT
  const getSortBy = useMemo(() => getUrlParams(config.key.sortBy), [
    getUrlParams,
    config.key.sortBy,
  ])
  ...
  // PAGE
  const getPage = useMemo(() => getUrlParams(config.key.page) || 1, [
    getUrlParams,
    config.key.page,
  ])
  ...
  // PERPAGE
  const getPerPage = useMemo(() => getUrlParams(config.key.perPage), [
    getUrlParams,
    config.key.perPage,
  ])
  ...
  const clearQuery = useCallback(
    (keys = [], options) => {
      const next = Object.entries(urlParams).reduce(
        (prev, [key, value]) =>
          keys.includes(key) ? { ...prev, [key]: value } : prev,
        {}
      )
      applyQueryParams(next, options)
    },
